...
class NFSService(SimpleService):
    ...
    async def _start_freebsd(self):
        await self._freebsd_service("rpcbind", "start", quiet=True)
        await self.middleware.call("nfs.setup_v4")
        await self._freebsd_service("mountd", "start", quiet=True)
        await self._freebsd_service("nfsd", "start", quiet=True)
        await self._freebsd_service("statd", "start", quiet=True)
        await self._freebsd_service("lockd", "start", quiet=True)
